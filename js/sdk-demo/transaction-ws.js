...
async function testSDK() {
  ...
  try {
    ...
    // Use websocket instead of HTTP to listen for the transaction result
    await sdk.wsService.connect()
    await sdk.wsService.registerTx(txSigned)
    const [txID, dErr, result, error] = await sdk.wsService.listenTx()
    if (dErr) {
      throw dErr
    }
    if (error) {
    ...
    if (txID !== txSigned.id().toString()) {
      throw new Error(`Unexpected transaction: ${txID}`)
    }
